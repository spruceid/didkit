    ...
    jstring_or_error(
        ...
        (|| {
            ...
            let vc_string = match proof_format {
                ProofFormat::JWT => rt.block_on(credential.generate_jwt(
                    Some(&key),
                    &options.ldp_options,
                    resolver,
                ))?,
    ...
    jstring_or_error(
        ...
        (|| {
            ...
            let vp_string = match proof_format {
                ProofFormat::JWT => rt.block_on(presentation.generate_jwt(
                    Some(&key),
                    &options.ldp_options,
                    resolver,
                ))?,
...
pub extern "system" fn Java_com_spruceid_DIDKit_resolveDID(
    ...
    jstring_or_error(
        ...
        (|| {
            ...
            let input_metadata: ResolutionInputMetadata =
                serde_json::from_str(&input_metadata_json)?;
            let resolver = DID_METHODS.to_resolver();
            let rt = runtime::get()?;
            let (res_meta, doc_opt, doc_meta_opt) =
                rt.block_on(resolver.resolve(&did, &input_metadata));
...
pub extern "system" fn Java_com_spruceid_DIDKit_dereferenceDIDURL(
    ...
    jstring_or_error(
        ...
        (|| {
            ...
            let input_metadata: DereferencingInputMetadata =
                serde_json::from_str(&input_metadata_json)?;
            let resolver = DID_METHODS.to_resolver();
            let rt = runtime::get()?;
